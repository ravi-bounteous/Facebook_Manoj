import { Router, Request, Response } from "express";
import * as authService from "../services/authService";
import * as passwordResetService from "../services/passwordResetService";
import {
  AccountLockedError,
  DuplicateEmailError,
  InvalidCredentialsError,
  InvalidResetTokenError,
  ValidationError,
} from "../services/errors";

const GENERIC_RESET_REQUEST_MESSAGE = "If that email is registered, a reset link has been sent.";

export const authRouter = Router();

authRouter.post("/register", async (req: Request, res: Response) => {
  const startedAt = Date.now();
  const { email, password } = req.body ?? {};
  try {
    const result = await authService.register(email, password);
    res.status(201).json(result);
    console.info(`[metrics] route=register status=201 duration_ms=${Date.now() - startedAt}`);
  } catch (err) {
    if (err instanceof ValidationError) {
      res.status(400).json({ error: err.message });
      console.info(`[metrics] route=register status=400 duration_ms=${Date.now() - startedAt}`);
      return;
    }
    if (err instanceof DuplicateEmailError) {
      res.status(409).json({ error: err.message });
      console.info(`[metrics] route=register status=409 duration_ms=${Date.now() - startedAt}`);
      return;
    }
    console.error(`[metrics] route=register status=500 duration_ms=${Date.now() - startedAt}`);
    throw err;
  }
});

authRouter.post("/login", async (req: Request, res: Response) => {
  const startedAt = Date.now();
  const { email, password } = req.body ?? {};
  try {
    const result = await authService.login(email, password);
    res.status(200).json(result);
    console.info(`[metrics] route=login status=200 duration_ms=${Date.now() - startedAt}`);
  } catch (err) {
    if (err instanceof AccountLockedError) {
      res.status(423).json({ error: err.message });
      console.info(`[metrics] route=login status=423 duration_ms=${Date.now() - startedAt}`);
      return;
    }
    if (err instanceof InvalidCredentialsError) {
      res.status(401).json({ error: err.message });
      console.info(`[metrics] route=login status=401 duration_ms=${Date.now() - startedAt}`);
      return;
    }
    console.error(`[metrics] route=login status=500 duration_ms=${Date.now() - startedAt}`);
    throw err;
  }
});

authRouter.post("/password-reset/request", async (req: Request, res: Response) => {
  const startedAt = Date.now();
  const { email } = req.body ?? {};
  try {
    await passwordResetService.requestReset(email);
    res.status(200).json({ message: GENERIC_RESET_REQUEST_MESSAGE });
    console.info(`[metrics] route=password-reset/request status=200 duration_ms=${Date.now() - startedAt}`);
  } catch (err) {
    if (err instanceof ValidationError) {
      res.status(400).json({ error: err.message });
      console.info(`[metrics] route=password-reset/request status=400 duration_ms=${Date.now() - startedAt}`);
      return;
    }
    console.error(`[metrics] route=password-reset/request status=500 duration_ms=${Date.now() - startedAt}`);
    throw err;
  }
});

authRouter.post("/password-reset/confirm", async (req: Request, res: Response) => {
  const startedAt = Date.now();
  const { token, password } = req.body ?? {};
  if (!token || typeof token !== "string") {
    res.status(400).json({ error: new InvalidResetTokenError().message });
    console.info(`[metrics] route=password-reset/confirm status=400 duration_ms=${Date.now() - startedAt}`);
    return;
  }
  try {
    await passwordResetService.resetPassword(token, password);
    res.status(200).json({});
    console.info(`[metrics] route=password-reset/confirm status=200 duration_ms=${Date.now() - startedAt}`);
  } catch (err) {
    if (err instanceof InvalidResetTokenError) {
      res.status(400).json({ error: err.message });
      console.info(`[metrics] route=password-reset/confirm status=400 duration_ms=${Date.now() - startedAt}`);
      return;
    }
    if (err instanceof ValidationError) {
      res.status(400).json({ error: err.message });
      console.info(`[metrics] route=password-reset/confirm status=400 duration_ms=${Date.now() - startedAt}`);
      return;
    }
    console.error(`[metrics] route=password-reset/confirm status=500 duration_ms=${Date.now() - startedAt}`);
    throw err;
  }
});

authRouter.post("/refresh", async (req: Request, res: Response) => {
  const startedAt = Date.now();
  const { refreshToken } = req.body ?? {};
  try {
    const result = await authService.refresh(refreshToken);
    res.status(200).json(result);
    console.info(`[metrics] route=refresh status=200 duration_ms=${Date.now() - startedAt}`);
  } catch {
    res.status(401).json({ error: "Invalid or expired refresh token" });
    console.info(`[metrics] route=refresh status=401 duration_ms=${Date.now() - startedAt}`);
  }
});
